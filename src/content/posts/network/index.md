...
| 网络主题 | 关联的 C++ 知识 |
|---------|---------------|
| TCP 粘包的缓冲区管理 | → [C++ Ch1 内存模型](../cpp_deep_dive/01_memory_model)（缓冲区分配） |
| KCP 使用 | → [C++ Ch7 并发](../cpp_deep_dive/07_concurrency)（SPSC 无锁队列传包） |
| Socket 的 RAII 封装 | → [C++ Ch2 智能指针](../cpp_deep_dive/02_pointers_and_smart_pointers)（RAII 思想） |
| IO 多路复用 + 线程池 | → [C++ Ch7 并发](../cpp_deep_dive/07_concurrency)（线程池实现） |
| Protobuf 序列化 | → [C++ Ch5 模板](../cpp_deep_dive/05_templates)（泛型序列化） |
